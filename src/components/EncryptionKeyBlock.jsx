...
const EncryptionKeyBlock = ({ activeStep, isActive }) => {
  ...
  return (
    <div
      className={`bg-slate-900/50 border rounded-xl p-6 relative overflow-hidden transition-all duration-500 ${
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-4">
        <div
          className={`p-2 rounded-lg transition-colors ${
            isActive || activeStep > 3 ? "bg-cyan-500/20" : "bg-slate-800"
          }`}
        >
          <Key
            className={`w-5 h-5 ${
              isActive || activeStep > 3 ? "text-cyan-400" : "text-slate-600"
            }`}
          />
        </div>
        <h3
      ...
      <div className="bg-black/40 rounded-lg p-4 font-mono text-xs border border-slate-800/50 min-h-[70px] flex items-center">
        <span
          className={`break-all ${
            isProcessing ? "text-cyan-400/50" : "text-cyan-400"
          }`}
        >
          {key}
          {isProcessing && (
      ...
      <div className="mt-4 flex justify-between items-center text-[10px]">
        <span className="text-slate-600 tracking-widest font-bold">
          AES-256-GCM
        </span>
        <span
