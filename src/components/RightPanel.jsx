    ...
    <div className="w-[35%] bg-white border border-gray-200 rounded-xl p-6 shadow-lg">
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2 pt-4 mt-4 border-t border-gray-200">
          {/* <button
            className="flex-1 bg-gray-100 text-gray-700 py-2 rounded-lg font-medium text-sm transition-all hover:bg-gray-200 hover:shadow-sm"
            onClick={() => {
              const users = JSON.parse(
                localStorage.getItem("production-users") || "[]"
              );
              alert(
                `Check console for storage data. Found ${users.length} user(s).`
              );
            }}
          >
            View Storage
